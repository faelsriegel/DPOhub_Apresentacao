import DescriptionItem from "@/components/ui/description-item"; // Componente responsável por renderizar os itens da lista
import { Typography } from "@/typography"; // Componente de tipografia para títulos e descrições
import { ShieldCheck, Database, FileText } from "lucide-react"; // Ícones relacionados ao contexto de LGPD e DPO
...
export const FirstSection: React.FC = () => {
  ...
  const gradientIconClass =
    "text-transparent bg-clip-text bg-gradient-to-r from-[#5276a0] via-[#89adcc] to-[#868585]";

  // Definição dos itens da seção, incluindo ícones, títulos, subtítulos e descrições
  const items = [
    {
      icon: <ShieldCheck className={`h-10 w-10 ${gradientIconClass}`} />, // Ícone de escudo com gradiente
    ...
    {
      icon: <Database className={`h-10 w-10 ${gradientIconClass}`} />, // Ícone de banco de dados com gradiente
    ...
    {
      icon: <FileText className={`h-10 w-10 ${gradientIconClass}`} />, // Ícone de arquivo com gradiente
    ...
    <div className="w-full border-b border-slice-line lg:py-20 py-14 lg:px-0 px-14">
      ...
      <section className="container mx-auto">
        ...
        <Typography
          ...
          className="text-primary leading-[1]" // Ajuste para espaçamento entre linhas
        >
        ...
        {/* Grade para exibição dos itens explicativos */}
        <div className="sm:grid sm:grid-cols-1 lg:grid-cols-3 gap-8 mt-20 flex flex-col">
          {items.map((item, index) => (
            <DescriptionItem
              ...
              subtitle={item.subtitle} // Subtítulo do item
              description={item.description} // Descrição detalhada do item
              icon={item.icon} // Ícone do item com gradiente
            />
          ))}
        </div>
      </section>
